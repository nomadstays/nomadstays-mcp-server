...
export async function getStaysByLifestyle(connStr: string, opts?: { lifestyle?: string | null, limit?: number }): Promise<Stay[]> {
  ...
  try {
    ...
    console.error(`getStaysByLifestyle connecting (sanitized): lifestyle=${lifestyle ?? 'null'} limit=${limit} conn=${masked}`);
    const parsed = parseConnectionString(connStr);
    console.error(`getStaysByLifestyle parsed config preview: server=${parsed.config.server ?? ''} port=${parsed.config.port ?? ''} db=${parsed.config.database ?? ''}`);
  } catch (e) {
    console.error('getStaysByLifestyle logging failed:', e);
  }
  ...
  try {
    ...
    // Quick check: ensure expected tables exist to provide helpful error if not
    const check = await req.query("SELECT OBJECT_ID('dbo.tbStays') AS objId, OBJECT_ID('dbo.tbGenre') AS genreObjId");
    if (!check.recordset || !check.recordset[0] || !check.recordset[0].objId || !check.recordset[0].genreObjId) {
      ...
      throw new Error(`Expected tables 'tbStays' or 'tbGenre' not found in database. Available tables: ${found.join(', ')}. Ensure the connection string points to the correct database and the schema/objects exist.`);
    }

    req.input('limit', sql.Int, limit);
    
    // Lifestyle filter: matches GenreTitle from tbGenre table
    // The Genre field in tbStays contains comma-separated GenreIDs (e.g., '1,3,5')
    // We need to join where GenreID from tbGenre matches any ID in the comma-separated list
    let whereLifestyle = '';
    if (lifestyle) {
      const searchTerm = String(lifestyle).trim();
      req.input('lifestyleSearchPattern', sql.VarChar(100), `%${searchTerm}%`);
      whereLifestyle = `AND G.GenreTitle LIKE @lifestyleSearchPattern`;
    }
    ...
    const query = `
      SELECT DISTINCT TOP (@limit)
        ...
        G.GenreTitle AS LifestyleName,
        G.GenreID AS LifestyleID,
        ...
        CASE WHEN S.WiFi_Download_Speed IS NULL OR S.WiFi_Download_Speed = '' THEN 1 ELSE 0 END AS WiFiSpeedNullSort,
        CASE WHEN P.MinSellPrice IS NULL THEN 1 ELSE 0 END AS MinPriceNullSort
      FROM tbStays S
      LEFT JOIN tbCountry CO ON S.Country = CO.CountryId
      LEFT JOIN tbGenre G ON ',' + S.Genre + ',' LIKE '%,' + CAST(G.GenreID AS nvarchar(20)) + ',%'
      ...
      try {
        ...
        try {
          ...
          r.summary = {
            ...
            lifestyle: r.LifestyleName ?? null
          };
        } catch { /* ignore */ }

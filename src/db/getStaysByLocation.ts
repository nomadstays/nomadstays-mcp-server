...
export async function getStaysByLocation(connStr: string, opts?: { location?: string | null, limit?: number }): Promise<Stay[]> {
  ...
  try {
    ...
    console.error(`getStaysByLocation connecting (sanitized): location=${location ?? 'null'} limit=${limit} conn=${masked}`);
    const parsed = parseConnectionString(connStr);
    console.error(`getStaysByLocation parsed config preview: server=${parsed.config.server ?? ''} port=${parsed.config.port ?? ''} db=${parsed.config.database ?? ''}`);
  } catch (e) {
    console.error('getStaysByLocation logging failed:', e);
  }
  ...
  try {
    ...
    // Location filter: flexible search that matches across City, State, location_name, location_country, or location_description
    let whereLocation = '';
    if (location) {
      ...
      whereLocation = `AND (
        ...
        OR L.location_name LIKE @locationSearchPattern 
        OR L.location_country LIKE @locationSearchPattern 
        OR L.location_description LIKE @locationSearchPattern
      )`;
    }
